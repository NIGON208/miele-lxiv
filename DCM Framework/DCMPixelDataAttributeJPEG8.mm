...
#include "dcmtk/dcmjpeg/libijg8/jinclude8.h"
...
jpeg8_nsdata_src (j_decompress_ptr cinfo, NSData *aData)
{
  ...
  if (cinfo->src == NULL) {	/* first time for this JPEG object? */
      cinfo->src = (struct jpeg_source_mgr *)
                   ...
                   ((j_common_ptr) cinfo, JPOOL_PERMANENT, SIZEOF(data8_source_mgr));
...
jpeg8_NSData_dest (j_compress_ptr cinfo, NSMutableData *aData)
{
  ...
  if (cinfo->dest == NULL) {	/* first time for this JPEG object? */
      cinfo->dest = (struct jpeg_destination_mgr *)
                    (*cinfo->mem->alloc_small)
                    ((j_common_ptr) cinfo, JPOOL_PERMANENT, SIZEOF(data8_dst_mgr));
...
//	}
    ...
    theWrkCh8P = (unsigned char *)[rawData mutableBytes];
...
- (NSMutableData *)compressJPEG8:(NSMutableData *)data
               compressionSyntax:(DCMTransferSyntax *)compressionSyntax

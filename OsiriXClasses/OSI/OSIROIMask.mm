...
const OSIROIMaskRun OSIROIMaskRunZero = {{0, 0}, 0, 0, 1.0};
...
{
    const OSIROIMaskRun* maskRun1 = (const OSIROIMaskRun*)voidMaskRun1;
    const OSIROIMaskRun* maskRun2 = (const OSIROIMaskRun*)voidMaskRun2;
...
+ (instancetype)ROIMaskWithBoxWidth:(NSUInteger)width height:(NSUInteger)height depth:(NSUInteger)depth;
{
    ...
    OSIROIMaskRun *maskRuns = (OSIROIMaskRun *)malloc(width * height * sizeof(OSIROIMaskRun));
...
+ (instancetype)ROIMaskWithElipsoidWidth:(NSUInteger)width height:(NSUInteger)height depth:(NSUInteger)depth
{
    ...
    OSIROIMaskRun *maskRuns = (OSIROIMaskRun *)malloc(height * depth * sizeof(OSIROIMaskRun));
...
- (instancetype)initWithIndexes:(NSArray *)maskIndexes
{
    ...
    OSIROIMaskIndex *maskIndexArray = (OSIROIMaskIndex *)[maskData mutableBytes];
    NSUInteger i;
...
- (OSIROIMask *)ROIMaskByTranslatingByX:(NSInteger)x Y:(NSInteger)y Z:(NSInteger)z
{
    ...
    OSIROIMaskRun *newMaskRuns = (OSIROIMaskRun *)malloc(maskRunCount * sizeof(OSIROIMaskRun));
...
//    OSIROIMaskRun run2;
    ...
    const OSIROIMaskRun *maskRunArray1 = (const OSIROIMaskRun *)[maskRun1Data bytes];
    const OSIROIMaskRun *maskRunArray2 = (const OSIROIMaskRun *)[maskRun2Data bytes];
...
- (OSIFloatVolumeData *)floatVolumeDataRepresentationWithVolumeTransform:(N3AffineTransform)volumeTransform;
{
    ...
    float *floatBytes = (float *)calloc(width * height * depth, sizeof(float));
    if (floatBytes == 0) {
...
- (OSIROIMask *)ROIMaskBySubtractingMask:(OSIROIMask *)subtractMask
{
    ...
    const OSIROIMaskRun *subtractRunArray = (const OSIROIMaskRun *)[subtractData bytes];
...
- (OSIROIMask *)ROIMaskCroppedToWidth:(NSUInteger)width height:(NSUInteger)height depth:(NSUInteger)depth
{
    ...
    OSIROIMaskRun *newMaskRuns = (OSIROIMaskRun *)malloc(newMaskRunsCount * sizeof(OSIROIMaskRun));
...
- (NSArray *)maskRuns 
{
    if (_maskRuns == nil) {
        ...
        const OSIROIMaskRun *maskRunArray = (const OSIROIMaskRun *)[_maskRunsData bytes];
...
- (NSData *)maskRunsData
{
    ...
    if (_maskRunsData == nil) {
        maskRunArray = (OSIROIMaskRun *)malloc([_maskRuns count] * sizeof(OSIROIMaskRun));
...
- (NSUInteger)maskIndexCount
{
    ...
    const OSIROIMaskRun *maskRunArray = (const OSIROIMaskRun *)[maskRunData bytes];
    NSUInteger maskRunCount = [self maskRunCount];
    NSUInteger maskIndexCount = 0;
    
    for (NSUInteger i = 0; i < maskRunCount; i++)
...
- (BOOL)containsIndex:(OSIROIMaskIndex)index;
{
    ...
    const OSIROIMaskRun *maskRuns = (const OSIROIMaskRun *)[maskRunsData bytes];
...
- (N3Vector)centerOfMass
{
    ...
    const OSIROIMaskRun *runArray = (const OSIROIMaskRun *)[maskData bytes];
...
- (NSString *)description
{
    ...
    const OSIROIMaskRun *maskRuns = (const OSIROIMaskRun *)[[self maskRunsData] bytes];
    for (NSUInteger i = 0; i < maskRunsCount; i++) {

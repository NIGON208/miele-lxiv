...
- (id) initCurveFitterWithXData: (double*) xD
                       andYData: (double*) yD
                         length: (int) l
{
	self = [super init];
	...
	xData = (double *)malloc( numPoints * sizeof( double));
	yData = (double *)malloc( numPoints * sizeof( double));
...
- (void) doFit: (int) fitType
{
	...
	double *center = (double *)malloc( sizeof( double) * numParams);  // mean of simplex vertices

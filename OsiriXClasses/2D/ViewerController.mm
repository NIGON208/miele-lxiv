    ...
    if ([self isDataVolumicIn4D: YES checkEverythingLoaded: YES tryToCorrect: YES checkForSliceInterval: YES] == NO)
    {
        ...
        else
        {
            NSRunAlertPanel(NSLocalizedString(@"Data Error", nil),
                            NSLocalizedString(@"This tool works only with 3D data series.", nil),
                            nil,
                            nil,
                            nil);
            return;
...
- (void) computeContextualMenu
{
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:OsirixPopulatedContextualMenuNotification
                                                        object:menu
...
-(NSMenu*)contextualMenuForROI:(ROI*)roi
{
	...
	[menu addItem: [[[NSMenuItem alloc] initWithTitle: [NSString stringWithFormat: NSLocalizedString( @"ROI: %@", nil), [roi name]]
                                               action: NULL
                                        keyEquivalent: @""] autorelease]];
	
    [menu addItem:[NSMenuItem separatorItem]];
    
    m = [[[NSMenuItem alloc] initWithTitle: NSLocalizedString( @"ROI Info", nil)
                                    action: @selector( roiGetInfo:)
                             keyEquivalent: @""] autorelease];
    [m setTarget:self];
    [menu addItem:m];
    
    m = [[[NSMenuItem alloc] initWithTitle: NSLocalizedString( @"ROI Histogram", nil)
                                    action: @selector( roiHistogram:)
                             keyEquivalent: @""] autorelease];
    [m setTarget:self];
    [menu addItem:m];
    ...
    if (roi.locked == NO)
    {
        ...
        m = [[[NSMenuItem alloc] initWithTitle: NSLocalizedString( @"Delete", nil)
                                        action: @selector(roiContextualMenuActionRemove:)
                                 keyEquivalent: @""] autorelease];
        [m setRepresentedObject:roi];
        [m setTarget:self];
        [menu addItem:m];
        
        m = [[[NSMenuItem alloc] initWithTitle: NSLocalizedString( @"Delete All With Same Name", nil)
                                        action: @selector(roiContextualMenuActionRemoveSameName:)
                                 keyEquivalent: @""] autorelease];
        [m setRepresentedObject:roi];
        [m setTarget:self];
        [menu addItem:m];
...
- (void)sendWillFreeVolumeDataNotificationWithVolumeData:(NSData *)freeingVolumeData movieIndex:(NSInteger)movieIndex
{
    if (freeingVolumeData) {
        ...
        [[NSNotificationCenter defaultCenter] postNotification:
         [NSNotification notificationWithName:OsirixViewerControllerWillFreeVolumeDataNotification
                                       object:self
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                               freeingVolumeData, @"volumeData",
                                               [NSNumber numberWithInteger:movieIndex], @"movieIndex",
                                               nil]]];
...
- (void)sendDidAllocateVolumeDataNotificationWithVolumeData:(NSData *)allocatingVolumeData movieIndex:(NSInteger)movieIndex
{
    if (allocatingVolumeData) {
        ...
        [[NSNotificationCenter defaultCenter] postNotification:
         [NSNotification notificationWithName:OsirixViewerControllerDidAllocateVolumeDataNotification
                                       object:self
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:
                                               allocatingVolumeData, @"volumeData",
                                               [NSNumber numberWithInteger:movieIndex], @"movieIndex",
                                               nil]]];
...
-(IBAction) setDefaultTool:(id) sender
{
	...
	if ([[buttonToolMatrix selectedCell] tag] == 0)
		...
		[[NSNotificationCenter defaultCenter] postNotificationName: OsirixDefaultRightToolModifiedNotification
                                                            object: sender
                                                          userInfo: [NSDictionary dictionaryWithObjectsAndKeys: @(ctag), @"toolIndex", nil]];
}
	...
	@synchronized( self)
	{
		...
		@try 
		{
			...
			else
			{
                ...
                if ([d count] > 0 &&
                    [fileList[ 0] count] > 0 &&
                    [[[[fileList[ 0] objectAtIndex: 0] valueForKey: @"series"] objectID] isEqualTo: [[[d objectAtIndex: 0] valueForKey: @"series"] objectID]])
                {
                    sameSeries = YES;
                }
...
{
	NSInteger result = NSRunCriticalAlertPanel(NSLocalizedString( @"Warning !", nil),
                                               NSLocalizedString( @"Modifying these parameters will:\r\r- Change the measurements results (length, surface, volume, ...)\r-Change the orientation of the slices and of the 3D objects (Left, Right, ...)\r-Change the aspect of the 3D images. It can introduce distortions.\r\rONLY change these parameters if you know WHAT and WHY you are doing it.", nil),
                                               NSLocalizedString( @"I agree", nil),
                                               NSLocalizedString( @"Cancel", nil),
                                               nil);
...
- (void) flipDataThread: (NSDictionary*) d
{
	...
	char *ptr = (char *)[[d objectForKey: @"ptr"] pointerValue];
...
- (void) SetThicknessInterval:(id) sender
{
	...
	for (long i = 0; i < 3; i++)
        [[customOrigin cellWithTag: i] setFloatValue: o[ i]];
	...
	@autoreleasepool {
        @try {
            for (int x = 0; x < maxMovieIndex; x++)
            {
                ...
                float *t = (float *)malloc( srcf.height * srcf.width * sizeof( float));
                if (t)
                {
                    ...
                    int from = [[dict objectForKey: @"from"] intValue];
                    int to = [[dict objectForKey: @"to"] intValue];
                    float *fkernel = (float *)[[dict objectForKey: @"kernel"] pointerValue];
...
//	NSBitmapFormat format = [bitmap bitmapFormat];
	...
	NSArray *newROIPoints = [theNewROI points];
	for (int i=0; i<[newROIPoints count]; i++)
...
- (IBAction) roiSetPixelsCheckButton:(id) sender
{
	...
	if (restoreAvailable == NO)
	{
        ...
        [[newValueMatrix cellWithTag: 1] setEnabled: YES];
}
...
- (IBAction) roiSetPixels:(ROI*)aROI :(short)allRois :(BOOL) propagateIn4D :(BOOL)outside :(float)minValue :(float)maxValue :(float)newValue :(BOOL) revert
{
	NSMutableArray	*roiToProceed = [NSMutableArray array],
                    *addedROIs = [NSMutableArray array];
			...
			for (int x = 0; x < [pixList[y] count]; x++)
			{
				if (allRois == 2)
				{
					...
					[roiToProceed addObject: [NSDictionary dictionaryWithObjectsAndKeys:
                                              ...
                                              nsoutside, @"outside",
                                              nsrevert, @"revert",
                                              [NSNumber numberWithInt: x], @"stackNo",
                                              nil]];
				}
				else
				{
                      ...
                      ratio:(float) ratio
{
    if (a.type == tMeasure && b.type == tMeasure)
    {
        ...
        [newMeasure addPoint:[ROI pointBetweenPoint:[a pointAtIndex:0]
                                           andPoint:[b pointAtIndex:0]
                                              ratio:ratio]];
        [newMeasure addPoint:[ROI pointBetweenPoint:[a pointAtIndex:1]
	...
	for (int i = 0; i < [aPts count]; i++)
	{
		...
		NSPoint newPt = [ROI pointBetweenPoint: [aP point]
                                      andPoint: [bP point]
                                         ratio: ratio];
...
- (void)bringToFrontROI:(ROI*) roi;
{
	...
	else // bring the whole group to front, without changing order inside the group
	{
        ...
        for (long i=(long)[group count]-1; i>=0; i--)
...
- (ROI*) convertBrushROItoPolygon:(ROI*) selectedROI numPoints: (int) numPoints
{
	...
	if ([selectedROI type] == tPlain)
	{
		...
		for (i = 0 ; i < numPoints ; i++)
		{
			...
			NSPoint c = [ROI pointBetweenPoint: [a point]
                                      andPoint: [b point]
                                         ratio: x - (float) xint];
...
-(NSImage*) imageForFrame:(NSNumber*) cur maxFrame:(NSNumber*) max
{
	...
	if (qt_dimension == 3)
	{
		...
		if ([imageView flippedData])
            index = (long)[[self fileList] count] -1 -curSample;
		else
            index = curSample;
...
- (IBAction) Panel3D:(id) sender
{
    ...
    if ([self isDataVolumicIn4D: YES checkEverythingLoaded: YES tryToCorrect: YES checkForSliceInterval: YES] == NO)
    {
        ...
        {
            NSRunAlertPanel(NSLocalizedString(@"Volume Rendering", nil),
                            NSLocalizedString(@"Volume Rendering requires volumic data.", nil),
                            nil,
                            nil,
                            nil);
            return;
	...
	else
	{
		...
		{
			viewer = [[VRController alloc] initWithPix:pixList[curMovieIndex]
                                                      :fileList[0]
                                                      :volumeData[ 0]
                                                      :blendingController
                                                      :self

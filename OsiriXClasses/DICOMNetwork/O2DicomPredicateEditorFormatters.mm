      ...
      errorDescription:(NSString**)error
{
    ...
    *obj = string;
    ...
    if (![s scanInteger:&num]) {
        if (error)
            *error = NSLocalizedString(@"This field must contain a numeric value.", nil);
        return NO;
    }
    
    if (num > 999 || num < 0) {
        if (error)
            *error = NSLocalizedString(@"The numeric value in this field must be between 0 and 999.", nil);
        return NO;
    }

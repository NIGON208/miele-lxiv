...
{
	if ( (self = [super init]) ) {
		_bezierCore	= (N3MutableBezierCoreRef)N3BezierCoreRetain(bezierCore);
	}
	return self;
}
...
- (NSArray*)intersectionsWithPlane:(N3Plane)plane relativePositions:(NSArray **)returnedRelativePositions;
{
	...
	intersections = (N3Vector *)malloc(intersectionCount * sizeof(N3Vector));
	relativePositions = (CGFloat *)malloc(intersectionCount * sizeof(CGFloat));
